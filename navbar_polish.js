...
files.forEach(file => {
    ...
    // 1. Reconstruct navbar__actions to ONLY have the search box
    // Targeting the entire div to ensure any theme/bookmark buttons are removed
    const navbarActionsRegex = /<div class="navbar__actions">([\s\S]*?)<\/div>/;

    const newActions = `<div class="navbar__actions">
    ...
    content = content.replace(navbarActionsRegex, newActions);

    // 2. Remove Bookmarks Panel aside if it exists
    const bookmarksPanelRegex = /<!-- Bookmarks Panel -->\s*<aside id="bookmarks-panel"[\s\S]*?<\/aside>/;
    const bookmarksPanelRegexNoComm = /<aside id="bookmarks-panel"[\s\S]*?<\/aside>/;

    content = content.replace(bookmarksPanelRegex, '');
    content = content.replace(bookmarksPanelRegexNoComm, '');
